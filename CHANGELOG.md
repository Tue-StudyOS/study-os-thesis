...
- Single-file portable edition (`thesis-finder-portable-vX.Y.Z.md`, plus the instructions
  block as a separate `.txt`), generated by `scripts/build_portable_bundle.py`. Runs in
  ChatGPT and Gemini, which do not load Agent Skills and cap a container at about ten
  knowledge files and 8000 characters of instructions.
- Session continuity for clients without a filesystem: the session log is now produced twice
  per run — once as soon as the profile interview is complete, again at the end — and is
  read back from attached files and container knowledge at the start of every run.

### Changed

- INSTALL.md is organised around three routes (Claude app, ChatGPT/Gemini, terminal agents)
  and a "Never type it all again" section that says, per platform, where the session file
  has to live.
