...
| Humanities (hard) | — | **60%** (3/5) | — (recall-only, no distinct strict pass run) | **100%** (3/3) | `2026-07-02-live-eval-runbook.md`, 2026-07-03 entry (Task Q run 1) — see caveat below |
| Law (hard) | — | not yet run | not yet run | not yet run | GT exists (`skills/tests/eval_ground_truth/law.md`), blind live run not yet exercised |
...
**Humanities caveat (important, not a skill defect):** the 60% recall is root-caused to the
...
an interest in the history of the field"). The live crawl found and evaluated all 5
ground-truth chairs; it then correctly excluded 2 of them per no-gos the *incomplete* persona
implied. Precision on what it did surface was 100%. Full account:
`2026-07-02-live-eval-runbook.md`, 2026-07-03 entry.

**What this table says about the roadmap's "core is done" bar (§4: recall ≥80% across ≥6
faculties incl. ≥1 hard faculty):** **not yet met.** Only 5 faculties have a live number at
all (Medicine, Psychology, WiSo, CS, Humanities); of those, Humanities — the one hard faculty
with a live result — sits at 60%, below the 80% bar (with the protocol caveat above). Law and
Theology have ground truth but no live run yet. Precision has only been formally measured for
2 of 5 faculties (CS, Humanities).
...
Recall and precision are strong wherever measured on the four "easy" faculties (all ≥83%
strict after fixes) and on companies (100%/94%). Steering is confirmed on one faculty.
Robustness edge cases (as opposed to hard-faculty recall) pass cleanly. The two genuine open
gaps, stated plainly: **(1)** the roadmap's own "core is done" recall bar (≥80% across ≥6
faculties incl. a hard one) is not yet met — Humanities is the only hard faculty with a live
number and it's below bar, Law/Theology are unrun; **(2)** output quality (Task S) has no
data at all. Company-eval circularity (§2) and single-agent/small-n scoring across every
table here are the standing methodological caveats already tracked in
