...
from app.schemas.chair import ArxivIngestRequest, ChairCreate, ChairPatch
...
class ChairService:
    ...
    async def ingest_arxiv_paper(
        ...
        _logger.info(
            "ArXiv ingest started: chair_id=%d arxiv_id=%r", chair_id, req.arxiv_id
        )
        chair = await self._chair_repo.get_by_id(chair_id)
        if chair is None:
            raise NotFoundException("Chair", chair_id)

        # Check per-chair uniqueness before hitting the ArXiv API.
        ...
        _logger.info(
            "ArXiv metadata fetched: title=%r year=%s abstract_len=%d",
            title, year, len(abstract),
        )

        _logger.info(
            "Creating embedding for ArXiv paper %r (model=%s)",
            req.arxiv_id, self._settings.ollama_embed_model,
        )
        embedding = await self._embed_text(abstract)
        if embedding is not None:
            _logger.info(
                "Embedding created for ArXiv paper %r: dim=%d", req.arxiv_id, len(embedding)
            )
        else:
            _logger.warning(
                "Embedding failed for ArXiv paper %r — stored without embedding", req.arxiv_id
            )
        ...
        await self._chair_repo.commit()
        _logger.info(
            "ArXiv paper %r inserted as document id=%d for chair_id=%d",
            req.arxiv_id, doc.id, chair_id,
        )
        return doc
    ...
    # ------------------------------------------------------------------
    # Vector search (used by the chat agent)
    # ------------------------------------------------------------------

    async def search_chairs(
        self, query_embedding: list[float], k: int = 5
    ) -> list[dict]:
        return await self._chair_repo.search_by_embedding(query_embedding, k=k)

    async def search_chairs_by_text(self, query: str, k: int = 5) -> list[dict]:
        embedding = await self._embed_text(query)
        return await self._chair_repo.search_by_embedding(embedding, k=k)
    ...
    async def _embed_text(self, text: str) -> list[float] | None:
        _logger.info(
            "Embedding text: model=%s text_len=%d", self._settings.ollama_embed_model, len(text)
        )
        try:
...
async def _fetch_arxiv_metadata(arxiv_id: str) -> tuple[str, str, int | None]:
    ...
    async with httpx.AsyncClient(timeout=15.0) as client:
        try:
            ...
            raise BadRequestException(
                f"Could not reach ArXiv API: {exc}"
            ) from exc
    ...
    if not abstract:
        raise BadRequestException(
            f"ArXiv paper '{arxiv_id}' has no abstract or was not found."
        )
